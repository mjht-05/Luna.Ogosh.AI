...
for (const lang of langs) {
    if (lang) {
        const key = lang.toLowerCase().replace(/ /g, '_').replace(/\(/g, '').replace(/\)/g, '');
        const code = codes[lang] || '';
        lines.push(`  ${key}: { label: '${lang}', code: '${code}' },`);
    }
}
lines.push("  hinglish: { label: 'Hinglish (HiEn)', code: 'hien' }");

console.log('const ALL_LANGS = {\n' + lines.join('\n') + '\n};');
